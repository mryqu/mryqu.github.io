...
![[Hue] 清空MySQL数据库的hue.django_content_type表时遇到由于外键约束无法删除错误！](/content/images/2016/8/0026uWfMzy77IKYxdW7bf.jpg) 默认Hue服务器使用嵌入式数据库SQLite存储元数据和查询信息。当我将其迁移到MySQL时，按照[Cloudera - Hue Installation Guide](https://archive.cloudera.com/cdh5/cdh/5/hue/manual.html)的步骤同步数据库时后清空MySQL中的django_content_type表时遭遇下列问题：
```
