...
![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75Irl45GHbb.jpg)![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75IrlBfUS4a.jpg)
### Google Sheets API鉴权
...
- 用于用户登录的HTTPGET请求如下（scope选择了profile、对文件元数据和内容只读访问、对表单和属性只读访问）：
  ...
  ```
  首先要求用户登录：![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75Irc6eVWf0.jpg)要求登录后用户的授权：![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75IrchpgWd6.jpg)返回页面包含授权码：![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75IrcpBpCe0.jpg)
- 获取访问令牌的HTTPPOST请求包含上面获得的授权码（在创建Google应用时获得的client_id和client_secret）：
  ...
  ```
  ![Google Sheets API认证和鉴权](/content/images/2016/9/0026uWfMzy75IrczJmDf7.jpg)
